...
def login_view(root):
    ...
    tk.Label(login_frame, text="Nombre:", font=("Arial", 12), bg="#f0f0f0", fg="#333").grid(row=1, column=0, sticky="w", pady=5)
    entry_nombre = tk.Entry(login_frame, font=("Arial", 12), width=25, bd=2, relief="solid")
    entry_nombre.grid(row=1, column=1, pady=5)

    tk.Label(login_frame, text="Contraseña:", font=("Arial", 12), bg="#f0f0f0", fg="#333").grid(row=2, column=0, sticky="w", pady=5)
    entry_password = tk.Entry(login_frame, font=("Arial", 12), width=25, bd=2, relief="solid", show="*")
    entry_password.grid(row=2, column=1, pady=5)

    # Función para cambiar el color del botón en el hover
    def on_enter(e):
